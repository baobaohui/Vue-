	...
	<div class="mui-card">
		...
		<div class="mui-card-content">
			<div class="mui-card-content-inner">
				...
				<!-- 关于获取numbox中子组件的数据，也就是子组件向父组件传值，这里
				 使用事件调用机制，父组件向子组件传递方法，子组件调用此方法，同时把数据当作参数
				 传递给这个方法-->
				</p>
			</div>
		</div>
...
export default{
	...
	methods:{
		getLunbotu(){
			this.$http.get('api/getthumimages/'+this.id).then(result=>{
				if(result.body.status === 0){
					//先循环轮播图数组的每一项，为item添加img属性，因为轮播图组件中
		...
		enter(el.done){
			el.offsetWidth;
			//小球回因为分辨率和未滚动的情况下而不准确，使用getBoundingClientRect()
			//方法动态获取二者差值，进行重新设置
		...
		getSelectedCount(count){
			//档子组件把选中的数量传递给父组件的时候，把选中的值保存到data上
